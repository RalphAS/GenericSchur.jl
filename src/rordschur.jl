...
function _trexchange!(S::AbstractMatrix{Ty}, T, Q, Z, iold, inew) where {Ty <: Real}
    ...
    if iold < inew
        ...
        @mydebug println(
            "forward swap $iold:$(iold + nbold - 1)"
        ...
        @mydebug println(
            "reverse swap $iold:$(iold + nbold - 1)"
                * " with $inew:$(inew + nbnew - 1)"
        )
        ihere = iold
        while ihere > inew
...
"""
function _swap1or2!(
        A::AbstractMatrix{Ty}, B, Q, Z, j1, n1, n2;
        require_strong = true
    ) where {Ty <: Real}
